...
// rollback target compatible with the additional learning-only namespace.
export function prepareLearningBootstrap(commit, next) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'er2-learning-baseline-'));
  ...
  try {
    ...
    if (baseline.name !== 'er2-lab-api' || baseline.main !== 'src/runtime.js' || baseline.keep_vars !== true ||
        !bindings.some(b => b.name === 'WEEKLY_WRITES' && b.class_name === 'WeeklyWriteCoordinator') || bindings.some(b => b.name === 'LEARNING_RECORDS'))
      throw Error('Unexpected baseline storage config; no learning migration performed');
    ...
    baseline.vars = { ...baseline.vars, LEARNING_RECORDS_ENABLED: 'false' };
    writeFileSync(configPath, JSON.stringify(baseline, null, 2) + '\n');
    const entry = join(root, 'worker/src/runtime.js');
    if (/LearningRecords/.test(readFileSync(entry, 'utf8'))) throw Error('Baseline already includes learning storage; inspect deployed binding');
    appendFileSync(entry, '\nexport class LearningRecords { async fetch() { return new Response("Inactive learning baseline", { status: 503 }); } async alarm() {} }\n');
    writeFileSync(join(root, 'worker/src/build-info.js'), 'export const BUILD_INFO = Object.freeze(' + JSON.stringify({ commit, builtAt: new Date().toISOString(), deploymentKind: 'learning-compatibility-baseline' }) + ');\n');

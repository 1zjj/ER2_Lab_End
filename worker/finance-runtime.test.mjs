...
const bundle=await build({stdin:{contents:`import runtime from ${JSON.stringify(entry)}; import { FinanceRecords as Actual } from ${JSON.stringify(finance)}; export { LearningRecords, WeeklyWriteCoordinator } from ${JSON.stringify(entry)}; export class FinanceRecords extends Actual { constructor(state,env){super(state,env);state.blockConcurrencyWhile(()=>state.storage.put('settings',{ready:true}));} } export default runtime;`,resolveDir:process.cwd()},bundle:true,write:false,format:'esm',platform:'browser'});

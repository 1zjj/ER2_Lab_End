...
export async function financeContext(request, env) {
  ...
  const actor=financeActor(people,session.sub);
  if(!isInternalMember(actor))throw authError(403,'财务流程仅向正式团队内成员开放');
  return {actor,people,access:financeAccess(actor,people,env)};
}

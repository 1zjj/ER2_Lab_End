...
export async function routeProjectConsistency(request, env) {
  try {
    ...
    return json(request, env, { version: PROJECT_MASTER_VERSION, checkedAt: new Date().toISOString(),
      ...
      definitions: {master: master.map(r=>({recordId:r.record_id,fields:Object.fromEntries(['项目编号','统一项目编号','ProjectID','项目名称','项目阶段','项目阶段（自动读取）','项目状态','状态','保密等级','是否启用','项目主页','项目负责人','成员'].filter(k=>k in (r.fields||{})).map(k=>[k,r.fields[k]]))})), mirrors:mirrors.map(r=>({recordId:r.record_id,fields:r.fields}))},

...
const original = process.cwd(), root = mkdtempSync(join(tmpdir(), 'er2-finance-bootstrap-test-'));
...
try {
  ...
  const bridge = prepareFinanceBootstrap(sha, next);
  try {
    assert.deepEqual(bridge.config.vars, { ...config.vars, FINANCE_ENABLED: 'false' });
    ...
    assert.ok(text.startsWith(runtime)); assert.ok(text.includes('export class FinanceRecords'));
  ...
  assert.throws(() => prepareFinanceBootstrap('invalid', next));

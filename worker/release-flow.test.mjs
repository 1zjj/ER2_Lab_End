...
try {
  ...
  writeFileSync(join(root, 'release-learning-bootstrap.mjs'), 'export function prepareLearningBootstrap() { return { configPath: "bridge.json", config: { vars: {} }, cleanup() {} }; }');
...
bindings.push({ name: 'LEARNING_RECORDS', type: 'durable_object_namespace', class_name: 'LearningRecords' });
...
globalThis.fetch = async (url, opts = {}) => {
  ...
  const learning = process.env.MOCK_MODE.startsWith('learning');
  const bootstrap = process.env.MOCK_MODE.startsWith('bootstrap') || learning;
  appendFileSync(process.env.MOCK_EVENTS, (opts.method || 'GET') + ' ' + new URL(url).pathname + '\\n');
  if (url.endsWith('/settings')) return Response.json({ success: true, result: { bindings: process.env.MOCK_MODE === 'binding' ? [] : (bootstrap && state === 'initial' ? bindings.filter(b => b.name !== (learning ? 'LEARNING_RECORDS' : 'WEEKLY_WRITES')) : bindings) } });
...
`);
  for (const mode of ['check', 'success', 'failed', 'binding', 'external', 'bootstrap', 'bootstrap-failed', 'learning', 'learning-failed']) {
    ...
    if (mode === 'bootstrap-failed' || mode === 'learning-failed') assert.equal(readFileSync(state, 'utf8'), 'bridge-restored');

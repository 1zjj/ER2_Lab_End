(function () {
  ...
  function create(options){
    ...
    async function summary(month){const m=month||new Date(Date.now()+8*3600000).toISOString().slice(0,7);shell('月度花费',`<label>采购月份<input type="month" data-finance-month value="${escape(m)}"></label>${button('load-summary','查看汇总')}<div data-finance-summary></div>`);await run(async()=>{const r=await api('/summary?month='+encodeURIComponent(m));dialog.querySelector('[data-finance-summary]').innerHTML=`<div class="finance-total">已确认报销<strong>${money(r.totalCents)}</strong></div><p>尚未确认：${money(r.pendingCents)}。按采购日期归属月份，历史迁入设备不计入本期新增报销。</p><p>${r.syncIssues?'有 '+r.syncIssues+' 张单据正在处理设备入库。':''}</p><div class="finance-table"><table><thead><tr><th>申报人</th><th>名称</th><th>数量</th><th>金额</th></tr></thead><tbody>${r.items.map(l=>`<tr><td>${escape(l.owner)}</td><td>${escape(l.name)}</td><td>${escape(l.quantity)}</td><td>${money(l.amountCents)}</td></tr>`).join('')}</tbody></table></div>`;});}
    async function setup(){shell('财务配置','<p>正在读取配置…</p>');await run(async()=>{const r=await api('/setup');shell('财务配置','<p>本配置只涉及预算与报销及指定的设备主表。</p><div class="finance-actions">'+button('inspect','只读核对源表与权限')+button('prepare','准备财务资料表')+'</div><label class="finance-check"><input type="checkbox" data-finance-dependencies>已核实原有工作流；迁移保留旧字段与关联结构</label><div class="finance-actions">'+button('migration-start','备份并启动设备迁移')+button('migration-step','继续下一步迁移')+'</div><details open><summary>当前核对结果</summary><pre data-finance-inspection>'+escape(JSON.stringify(r,null,2))+'</pre></details><p>启用前须完成设备复制及财务表原生权限检查。</p><label class="finance-check"><input type="checkbox" data-finance-native>已核实四张财务资料表仅财务及指定管理员可访问</label><label class="finance-check"><input type="checkbox" data-finance-reminders>启用每月20日、27日10:00提醒及次月1日10:00教授汇总（北京时间）</label><div class="finance-actions">'+button('activate','启用办理',true)+button('disable','暂停办理')+'</div>');});}
    async function setupAction(action){await run(async()=>{const r=await api('/setup',{action,nativeDependenciesVerified:dialog.querySelector('[data-finance-dependencies]')?.checked,nativePermissionsVerified:dialog.querySelector('[data-finance-native]')?.checked,reminders:dialog.querySelector('[data-finance-reminders]')?.checked});dialog.querySelector('[data-finance-inspection]').textContent=JSON.stringify(r,null,2);message('已完成当前操作。');mount();});}
    async function handle(action,el){
      ...
      if(action==='setup')return setup();if(['inspect','prepare','activate','disable','migration-start','migration-step'].includes(action))return setupAction(action);

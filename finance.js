(function () {
  ...
  function create(options){
    let meta=null,dialog=null,current=null,files=[],busy=false,dirty=false,listMode='mine',page=0,lastWrite=null,identity='',epoch=0,autoOpened=false,sessionEpoch=0,dialogEpoch=0,operation=0,reading=false,mountedRoot=null,retryRead=null;
    ...
    async function mount(reuse=false){
      ...
      try{const loaded=await api('',undefined,{card:true});if(generation!==epoch||!root.isConnected)return;meta=loaded;
        root.innerHTML='<div class="finance-actions">'+(meta.access.canSubmit?button('purchase','采购申请',false,meta.ready?'':'disabled')+button('claim','费用报销',true,meta.ready?'':'disabled')+button('records','我的记录'):'')+
          (meta.access.canReviewPurchase?button('purchase-review','采购待处理'+(meta.purchasePending?' · '+meta.purchasePending:'')):'')+(meta.access.canReview?button('review','报销审核'+(meta.pending?' · '+meta.pending:'')):'')+(meta.access.canSummary?button('summary','月度花费'):'')+(meta.access.canViewAll?button('all-records','全部单据'):'')+'</div>'+
    ...
    function renderDocument(d,attachments=[],history=[]){
      ...
      shell(claim?'费用报销':'采购申请',`<p class="finance-muted">${d.id?escape(d.id)+' · '+escape(meta.statuses[d.status])+' · '+escape(d.ownerName):'申报人：'+escape(profile().name)+'（按登录账号自动记录）'}</p>`+
        ...
        (meta.access.canReviewPurchase&&!claim&&d.status==='sent'&&!own?'<section class="finance-review"><h3>教师确认</h3><p>请核对购买内容、预计金额和用途。同意仅代表允许采购，不会产生报销或设备记录。</p><label>退回原因<textarea data-finance-reason maxlength="2000"></textarea></label><div class="finance-actions">'+button('purchase-return','退回修改')+button('purchase-approve','同意购买',true)+'</div></section>':'')+
        (meta.access.canReviewPurchase&&!claim&&d.status==='sent'&&own?'<p class="finance-muted">本人不能确认自己的采购申请。</p>':'')+
        ...
        '<div class="finance-actions">'+button('records','返回我的记录')+(meta.access.canReviewPurchase?button('purchase-review','返回采购待处理'):'')+(meta.access.canReview?button('review','返回报销审核'):'')+'</div>');
    ...
    async function listing(mode='mine',newPage=0){retryRead=()=>listing(mode,newPage);if(dirty&&!window.confirm('当前内容未保存，确定离开？'))return;dirty=false;listMode=mode;page=newPage;
      const title=mode==='all'?'全部单据':mode==='review'?'报销审核':mode==='purchase'?'采购待处理':'我的记录';
      const query='?review='+(mode==='review')+'&page='+page+(mode==='all'?'&all=true':mode==='purchase'?'&purchaseReview=true':'');
      shell(title,'<p>正在载入…</p>');await run(async()=>{const r=await api('/records'+query);shell(title,
        (mode==='review'?'<p>待审核报销需逐项核实。退回修改不入库；确认已报销后自动入库。</p>':mode==='purchase'?'<p>待确认采购由教师同意或退回；同意购买不会自动报销或登记设备。</p>':'')+
        '<div class="finance-records">'+(r.records.length?r.records.map(d=>`<article><div><strong>${escape(d.id)}</strong><p>${escape(d.kind==='claim'?'费用报销':'采购申请')} · ${escape(d.ownerName)} · ${escape(meta.statuses[d.status])}</p>${d.syncError?'<p class="finance-error">'+escape(d.syncError)+'</p>':''}${d.noticeError?'<p class="finance-muted">'+escape(d.noticeError)+'</p>':''}</div><div><strong>${money(d.totalCents)}</strong>${button('detail','查看',false,`data-id="${escape(d.id)}"`)}</div></article>`).join(''):'<p class="finance-muted">暂无记录。</p>')+'</div><div class="finance-actions">'+(page?button('prev','上一页'):'')+(r.more?button('next','下一页'):'')+button(mode==='all'?'all-records':mode==='review'?'review':mode==='purchase'?'purchase-review':'records','刷新列表')+'</div>');},true);}
    ...
    async function purchaseReview(action){if(action==='return'&&!dialog.querySelector('[data-finance-reason]').value.trim()){message('请填写退回原因。',true);return;}if(action==='approve'&&!window.confirm('确认同意这张采购申请？此操作不会产生报销或设备记录。'))return;
      const reason=dialog.querySelector('[data-finance-reason]')?.value||'';await run(async()=>{const r=await write('/purchase-review',{id:current.id,revision:current.revision,action,reason});renderDocument(r.document,files);message(action==='approve'?'已同意购买，并已通知申请人。':'已退回修改，并已通知申请人。');mountedRoot=null;mount();});}
    ...
    async function handle(action,el){
      ...
      if(action==='all-records')return listing('all');
      if(action==='records')return listing('mine');if(action==='review')return listing('review');if(action==='purchase-review')return listing('purchase');
      if(action==='next'||action==='prev')return listing(listMode,page+(action==='next'?1:-1));
      ...
      if(action==='reconcile')return run(async()=>{const r=await write('/reconcile',{});message('同步完成：'+(r.deleted?.length?'已隐藏 '+r.deleted.length+' 张后台删除单据。':'未发现新的后台删除单据。'));});
      ...
      if(action==='purchase-approve'||action==='purchase-return')return purchaseReview(action==='purchase-approve'?'approve':'return');

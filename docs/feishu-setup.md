...
可在发布目录使用 `prepare-weekly-automation.mjs <本地发布配置路径> <日志表完整知识库链接，包含 table 参数> --plan` 检查现有日志表。脚本从终端环境变量读取 `FEISHU_APP_SECRET`，不会输出密钥或发送消息。已配置接收人必须仍有接收职责；未配置时，仅在权威人员表中恰好有一名有效接收人时采用该人，遇到缺失或歧义停止处理。

把 `--plan` 改为 `--apply` 后，脚本先在本机私有备份目录保存原配置，再向专用日志表新增一条“周报自动化配置检查”记录并读回，确认应用确实可以写入。通过后原子更新本地发布配置，保留周报主表和其他设置。脚本不创建或删除表，不改人员职责；只有后续部署配置才会使既有定时任务具备运行条件。配置检查记录保留作验收证据，真实消息送达仍需另行验收。
